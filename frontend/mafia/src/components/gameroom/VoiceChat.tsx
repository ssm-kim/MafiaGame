...
function VoiceChat({ roomId, participantNo, nickname, gameState }: VoiceChatProps) {
  ...
  useEffect(() => {
    ...
    if (gameState?.roomStatus === 'PLAYING' && participantNo !== null && gameState.myInfo) {
      ...
      // 게임이 끝났거나 방을 나갔을 때 cleanup 실행
      cleanup();
    }

    // cleanup 함수를 외부로 분리
    function cleanup() {
      console.log('Running voice chat cleanup...');
      if (audioAnalyserInterval.current) {
        clearInterval(audioAnalyserInterval.current);
        audioAnalyserInterval.current = null;
      }
      ...
      if (session) {
        try {
          ...
          subscribers.forEach((subscriber) => {
            try {
              console.log('Unsubscribing stream:', subscriber.stream.streamId);
